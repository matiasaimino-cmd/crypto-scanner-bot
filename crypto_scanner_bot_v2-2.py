...
RSI_OVERSOLD   = 30
RSI_EXTREME    = 75
ALERTA_COOLDOWN_MIN = 60  # no repetir la misma alerta por 60 minutos
...
def ya_alerte(symbol, direction, timeframe):
    ...
    try:
        ...
        cur.execute("""
            ...
            AND enviada_at > NOW() - INTERVAL '%s minutes'
...
def guardar_estado(symbol, timeframe, rsi, estructura, htf_bias, divergencia, precio):
    ...
    try:
        ...
        """, (symbol, timeframe, rsi, str(estructura), htf_bias, str(divergencia), precio))
        conn.commit()
    except Exception as e:
...
def guardar_rsi(symbol, timeframe, rsi, precio):
    ...
    try:
        ...
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (symbol, timeframe, rsi, precio))
        # Mantener solo las ultimas 200 entradas por par
        cur.execute("""
    ...
    try:
        ...
        for i in range(5, min(len(df)-2, 200)):
            ...
            if c["close"] < c["open"] and ratio > 0.55:
                ...
                if fh > c["high"] and c["low"]*0.97 <= price <= c["high"]*1.03:
                    ob_bull = {"high": round(c["high"],6), "low": round(c["low"],6)}
            if c["close"] > c["open"] and ratio > 0.55:
                fl = df["low"].iloc[i+1:end].min()
                if fl < c["low"] and c["low"]*0.97 <= price <= c["high"]*1.03:
...
def analyze_symbol(symbol, interval):
    ...
    try:
        ...
        # DEBUG — log RSI de cada activo
        print("DEBUG " + symbol + " [" + interval + "] RSI:" + str(rsi) + " HTF:" + htf_bias)
        ...
        for direction, ob, fvg, near in [("LONG",ob_b,fv_b,near_sup),("SHORT",ob_s,fv_s,near_res)]:
            ...
            print("  " + direction + " score:" + str(score) + " labels:" + str(labels[:2]))
            if score >= MIN_SCORE:
...
def scan_all():
    ...
    msg += "Score minimo: " + str(MIN_SCORE) + "/10 | Cooldown: " + str(ALERTA_COOLDOWN_MIN) + "min\n"
    msg += "---\n"
...
if __name__ == "__main__":
    ...
    init_db()
    send_telegram(
        "<b>Bot Scanner Crypto v4.0 ACTIVO</b>\n\n"
        "Nuevo en v4.0:\n"
        "- Memoria PostgreSQL\n"
        "- No repite alertas (cooldown 60min)\n"
        "- Guarda historial de todas las senales\n"
