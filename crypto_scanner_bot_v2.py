...
def scan_all():
    ...
    # DEBUG — mostrar scores de todos los activos
    debug_msg = "<b>DEBUG scores BTC/ETH/SOL:</b>\n"
    for symbol in ["BTCUSDT","ETHUSDT","SOLUSDT","HBARUSDT"]:
        for interval, tf_label in INTERVALS:
            df = get_klines(symbol, interval, 200)
            if df is not None:
                rsi = calc_rsi(df["close"])
                ...
                divergence = detect_rsi_divergence(df)
                htf_bias   = get_htf_bias(symbol, interval)
                near_sup   = abs(df["close"].iloc[-1] - sup) / df["close"].iloc[-1] < 0.005
                near_res   = abs(df["close"].iloc[-1] - res) / df["close"].iloc[-1] < 0.005
                score_l, _ = calc_score("LONG",  rsi, ob_b, fv_b, structure, candles, vol_h, near_sup, divergence, htf_bias)
                score_s, _ = calc_score("SHORT", rsi, ob_s, fv_s, structure, candles, vol_h, near_res, divergence, htf_bias)
                debug_msg += symbol + " [" + tf_label + "] RSI:" + str(rsi) + " L:" + str(score_l) + " S:" + str(score_s) + "\n"
            time.sleep(0.1)
